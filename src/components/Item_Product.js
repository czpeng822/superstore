...
export default function Product() {
  ...
  if(selectedTab=== 'product'){
    productsdataByTab = products;
  }else{
    productsdataByTab = products.filter(product => product.categories.toLowerCase().includes(selectedTab));
  }
  
  const handleTabChange = (event, newValue) => {
    const tabNames = ['product', 'electronics', 'furniture', 'grocery','officesupplies','clothing','appliances'];
  ...
  return (
    <Tabs defaultValue={0} onChange={handleTabChange} style={{minWidth:'1350px', marginLeft: '30px'}}>
      <TabsList>
        <Tab value={0}>All product</Tab>
        <Tab value={1}>Electronics</Tab>
        <Tab value={2}>Furniture</Tab>
        <Tab value={3}>Grocery</Tab>
        <Tab value={4}>Office Supplies</Tab>
        <Tab value={5}>Clothing</Tab>
        <Tab value={6}>Appliances</Tab>
      </TabsList>
      ...
      <TabPanel value={5}><DisplayItem cartItems={cartItems} setCartItems={setCartItems} productsdataByTab={productsdataByTab}/></TabPanel>
      <TabPanel value={6}><DisplayItem cartItems={cartItems} setCartItems={setCartItems} productsdataByTab={productsdataByTab}/></TabPanel>
    </Tabs>
  );
}
