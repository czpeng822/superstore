...
export default function Phones({cartItems,setCartItems}) {
  const { products } = useContext(AppContext);
  const [selectedTab, setSelectedTab] = useState('phones');
  ...
  if(selectedTab=== 'phones'){
    ...
    productsdataByTab = products.filter(product => product.productName.toLowerCase().includes(selectedTab));
  }
  
  const handleTabChange = (event, newValue) => {
    const tabNames = ['phones', 'iphone', 'samsung'];
  ...
  return (
    <Tabs defaultValue={0} onChange={handleTabChange} style={{minWidth:'1350px', marginLeft: '30px'}}>
      <TabsList>
        <Tab value={0}>All Phones</Tab>
        <Tab value={1}>Iphone</Tab>
        <Tab value={2}>Samsung</Tab>
      </TabsList>
